...
(class IconBar of Set
  ...
  (method (handleEvent pEvent &tmp temp0 pEventType temp2 theCurIcon temp4 theCurInvIcon newEvent)
    (pEvent localize:)
    (cond
      ((& state $0004))
      (
        ...
        (if temp0
          (global1 setCursor: temp4 1 oldMouseX oldMouseY)
        else
          (global1
            setCursor:
              temp4
  ...
  (method (show &tmp temp0 temp1 temp2 temp3 theY temp5 temp6 temp7)
    ...
    (if curInvIcon
      (if (gEgo has: (global9 indexOf: curInvIcon))
        (= temp3
          (+
            (/
              (-
  ...
  (method (dispatchEvent pEvent &tmp pEventY pEventX pEventType pEventMessage theHighlightedIcon pEventClaimed temp6)
    (= pEventX (pEvent x?))
    (= pEventY (pEvent y?))
          ...
          (if
            ...
            (if (== theHighlightedIcon helpIconItem)
              (if (theHighlightedIcon cursor?)
                (global1 setCursor: (theHighlightedIcon cursor?))
              )
              (if (& state $0800)
                (self noClickHelp:)
              else
        ...
        (16384
          (if (== pEventMessage JOY_DOWNLEFT)
            ...
            (and theHighlightedIcon (theHighlightedIcon helpStr?))
              (= temp6 (GetPort))
              (proc255_4 937 0 (theHighlightedIcon helpStr?))
              (SetPort temp6)
            )
            (if helpIconItem
    ...
    (while (not ((= newEvent (Event new:)) type?))
      ...
      (cond
        ((= temp2 (self firstTrue: #onMe newEvent))
          (if
            (and
              (!= (= temp2 (self firstTrue: #onMe newEvent)) temp1)
              (temp2 helpStr?)
            )
            (= temp1 temp2)

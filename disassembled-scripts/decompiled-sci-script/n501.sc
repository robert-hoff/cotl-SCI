...
(instance jwitchct of Rm
  ...
  (method (init)
    ...
    ([newArrow 4] priority: 0)
    ([newArrow 5] priority: 0)
    ([newArrow 6] priority: 0)
    (switch global146
      (30 (= local60 20))
      (40 (= local60 20))
      (else  (= local60 15))
    )
    (if (and (== global151 0) (== global160 0))
      (= local60 20)
    )
    (if (or (!= global151 0) (!= global160 0))
      (knave init:)
    ...
    (gClient setScript: musicScript)
  )

  (method (dispose)
    (= local0 0)
    (while (< local0 8)
      (if (IsObject [newArrow local0])
        ([newArrow local0] dispose: delete:)
      )
      (++ local0)
    )
    (fireSound dispose:)
    (if (proc0_5 168) (proc0_6 61))
    (super dispose: &rest)
  )
)
...
(instance knavesDie of Script
  ...
  (method (changeState newState)
    (switch (= state newState)
      ...
      (1
        (theKnave stopUpd:)
        ([newArrow 1] init: addToPic:)
        (= ticks (* (= local52 (Random 3 6)) 20))
      )
      (2
        ([newArrow 6] init: addToPic:)
        ([newArrow 0] init: addToPic:)
        (theKnave2
          setScript: 0
          loop: 2
          cel: 0
      ...
      (3
        (theKnave2 stopUpd:)
        ([newArrow 4] init: addToPic:)
        (= ticks (* (= local52 (Random 1 3)) 10))
      )
      (4
        ([newArrow 3] init: addToPic:)
        ([newArrow 5] init: addToPic:)
        (theKnave3
          setScript: 0
          loop: 3
          cel: 0

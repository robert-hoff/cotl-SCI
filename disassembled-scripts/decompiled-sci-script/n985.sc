...
(class Avoid of Obj
  ...
  (method (doit &tmp theHeading theNearestDist clientX clientY theTargetX theTargetY clientMover temp7 temp8 clientMoverB_moveCnt temp10 temp11 temp12 temp13 temp14 temp15)
    ...
    (if (= clientMover (client mover?))
      ((= clientMover (client mover?)) setTarget:)
    )
    (cond
    ...
    (= theHeading
      (= heading
        (proc999_1 (* temp14 (/ heading temp14)) 360)
      )
    )
    (cond
      (
    ...
    (= temp11
      (<=
        (= temp8 (Abs (= temp7 (proc982_2 temp7 theHeading))))
        90
      )
    )
    (= thisTurn (if (proc999_0 temp7) else escapeTurn))
    ...
    (if escaping
      ...
      (while (and (< temp10 temp15) (self canBeHere:))
        (= heading
          (proc999_1
            (+ theHeading (* temp14 temp10 escapeTurn))
            360
          )
        )
  ...
  (method (incClientPos)
    (client
      x:
        (+
          (client x?)
          (* (proc999_0 (SinMult heading 100)) (client xStep?))
        )
      y:
        (-
          (client y?)
          (* (proc999_0 (CosMult heading 100)) (client yStep?))
        )
      heading: heading
    )
  )
  ...
  (method (canBeHere param1 param2 &tmp clientX clientY temp2)
    ...
    (= temp2
      (if
      (or (not (not (client cantBeHere:))) offScreenOK)
      else
